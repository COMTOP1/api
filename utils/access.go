...
func (a *Accesser) getAFCClaims(token string) (*AFCAccessClaims, error) {
	...
	if jwt1.Valid && claims.Issuer == "https://sso."+a.conf.DomainName && claims.Audience == "https://afcaldermaston.co.uk" && claims.IssuedAt == claims.NotBefore && claims.ExpiresAt > time.Now().Unix() {
		return claims, nil
	}
	return nil, ErrInvalidToken
}
